...
def get_profile(db: Session, current_user: models.User):
    ...
    elections = db.query(models.Election).order_by(models.Election.voting_start.asc()).all()
    election_sequence = {election.id: index for index, election in enumerate(elections)}
    ...
    vote_totals = (
        ...
        .filter(models.Election.result_visible == True, models.Election.voting_end <= now)
        .group_by(
            models.Vote.election_id,
            models.Vote.post_id,
            models.Vote.candidate_id,
