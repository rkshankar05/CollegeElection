...
def get_my_applications(db: Session, current_user: models.User):
    ...
    for application in applications.values():
        ...
        reviewed_at_values = [post["reviewed_at"] for post in application["posts"] if post["reviewed_at"]]
...
def apply_for_candidate(db: Session, current_user: models.User, data: schemas.CandidateApply):
    if current_user.role != "student":
        raise HTTPException(status_code=403, detail="Only students can apply")
    if len(data.post_ids) == 0:
        raise HTTPException(status_code=400, detail="Select at least one post")
    if len(data.post_ids) > 2:
        raise HTTPException(status_code=400, detail="You can apply for maximum 2 posts")
    ...
    if student.has_active_backlog:
        raise HTTPException(status_code=403, detail="Students with active backlog cannot apply")
    if student.candidate_blocked:
        raise HTTPException(status_code=403, detail=student.block_reason or "You are blocked by administration")
    ...
    if len(posts) != len(set(data.post_ids)):
        raise HTTPException(status_code=400, detail="Invalid post selected")

    candidate = models.Candidate(user_id=current_user.id, election_id=data.election_id)
    db.add(candidate)
    db.commit()
    db.refresh(candidate)

    for post_id in set(data.post_ids):
        db.add(models.CandidatePost(candidate_id=candidate.id, post_id=post_id, status="pending"))

    db.commit()
    return {"message": "Candidate application submitted successfully", "candidate_id": candidate.id}


def get_all_candidate_applications_for_election(db: Session, election_id: int):
    rows = (
        db.query(
...
def review_candidate(db: Session, candidate_post_id: int, data: schemas.CandidateReview):
    if data.status not in ["approved", "rejected"]:
        raise HTTPException(status_code=400, detail="Status must be approved or rejected")

    candidate_post = db.query(models.CandidatePost).filter(models.CandidatePost.id == candidate_post_id).first()
    if not candidate_post:
        raise HTTPException(status_code=404, detail="Candidate post not found")

    candidate = db.query(models.Candidate).filter(models.Candidate.id == candidate_post.candidate_id).first()
    if not candidate:
        ...
        approved_count = (
            ...
            .filter(
                ...
                or_(
                    ...
                    and_(models.CandidatePost.status.is_(None), models.Candidate.status == "approved"),
                ),
            )
            .count()
        )
        if approved_count >= 5:
            raise HTTPException(status_code=400, detail="This post already has 5 approved candidates")
...
def get_public_candidates(db: Session, election_id: int):
